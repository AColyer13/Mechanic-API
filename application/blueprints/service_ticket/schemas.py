...
class ServiceTicketSchema(SQLAlchemyAutoSchema):
    ...
    status = fields.Str(validate=validate.OneOf(['Open', 'In Progress', 'Completed', 'Cancelled']), default='Open')
    vehicle_year = fields.Int(validate=validate.Range(min=1900, max=2100))
    vehicle_vin = fields.Str(validate=validate.Length(equal=17))
